...
- 商品卡片高亮指标会跟随当前排序方式变化，灰色辅助指标会避开主指标避免重复
- 在浮窗中点击商品标题会定位到当前页面中的对应商品，不会打开新页面
- 浮窗打开后，noon 站内页面切换或商品内容异步更新时会自动刷新
- 支持详情页 `sold recently` 和列表/详情页 `Ratings` 销售热度识别，并标明来源
- 支持详情页评分旁独立数字形式的热度识别，例如 `4.7 23`
- 支持详情页评分和 ratings 数量分开渲染时的邻近 DOM 识别，减少热度未识别
- 详情页按 SKU 去重，避免切换销售指标时同一商品重复展示
- 列表页和详情页会用 SKU、标题、标题+价格、图片路径多维去重，减少重复行
- 生成本地规则建议
- 支持复制表格文本和导出 CSV
